import { l as parseWatch } from "./shipping-CitWW3XC.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/billets-ArYXOA2k.js
var VESSEL_BILLETS = [
	{
		code: "00",
		sortOrder: 0,
	...
	},
	{
		...
		sortOrder: 2,
		title: "Second Mate",
		shortTitle: "02 · Second Mate",
		department: "deck",
		watch: "12-4",
		unionHall: "MMP",
		defaultAssignment: "ROTARY"
	},
	{
		code: "03",
		sortOrder: 3,
		title: "Third Mate",
		shortTitle: "03 · Third Mate",
		department: "deck",
		watch: "8-12",
		unionHall: "MMP",
		defaultAssignment: "ROTARY"
	...
	},
	{
		code: "22",
		sortOrder: 22,
		title: "Steward Assist",
		shortTitle: "22 · Steward Assist",
		department: "steward",
		watch: "day",
		unionHall: "SIU",
		defaultAssignment: "ROTARY"
	...
	},
	{
		...
		sortOrder: 25,
		title: "Deck Cadet",
		shortTitle: "25 · Deck Cadet",
		department: "deck",
		watch: "day",
		unionHall: "NONE",
		defaultAssignment: "CADET"
	},
	{
		code: "26",
		sortOrder: 26,
		title: "Engine Cadet",
		shortTitle: "26 · Engine Cadet",
		department: "engine",
		watch: "day",
		unionHall: "NONE",
		defaultAssignment: "CADET"
	},
	{
		...
		sortOrder: 27,
		title: "Apprentice A",
		shortTitle: "27 · Apprentice A",
		department: "deck",
		watch: "day",
		unionHall: "NONE",
		defaultAssignment: "APPRENTICE"
	},
	{
		code: "28",
		sortOrder: 28,
		title: "Apprentice B",
		shortTitle: "28 · Apprentice B",
		department: "deck",
		watch: "day",
		unionHall: "NONE",
		defaultAssignment: "APPRENTICE"
...
function billetByCode(code) {
	if (!code) return void 0;
	return VESSEL_BILLETS.find((b) => b.code === code);
}
function scoreBillet(position, watch, b) {
	...
	for (const [re, code] of [
		...
		if (re.test(p) && [
			...
			"08",
			"09"
		].includes(b.code) && /AB\/W|AB W|WATCH/.test(p)) score += 10;
		...
		if (re.test(p) && [
			...
			"17",
			"18"
		].includes(b.code) && /QMED|QEE/.test(p)) score += 10;
	}
...
function matchBillets(position, occupiedCodes, watch) {
	if (!position?.trim()) return [];
	return VESSEL_BILLETS.map((b) => ({
		billet: b,
		score: scoreBillet(position, watch ?? parseWatch(position), b),
		vacant: !occupiedCodes.has(b.code)
	})).filter((x) => x.score > 0).sort((a, b) => b.score - a.score || Number(b.vacant) - Number(a.vacant));
}
function proposeBillet(position, occupiedCodes, watch) {
...
function canMoveBillet(fromCode, toCode) {
...
function canTradeBillet(fromCode, toCode) {
	return canMoveBillet(fromCode, toCode);
}
function listBilletChanges(fromCode, aboard, selfId) {
	if (!fromCode || !billetByCode(fromCode)) return [];
	if ((aboard.find((p) => p.id === selfId)?.assignmentType ?? "").toUpperCase() === "PERMANENT") return [];
	const sitting = /* @__PURE__ */ new Map();
	...
	for (const b of VESSEL_BILLETS) {
		...
		} else if (canMoveBillet(fromCode, b.code)) vacant.push({
			kind: "vacant",
			code: b.code,
...
//#endregion
export { listBilletChanges as a, canTradeBillet as i, billetByCode as n, proposeBillet as o, canMoveBillet as r, VESSEL_BILLETS as t };
