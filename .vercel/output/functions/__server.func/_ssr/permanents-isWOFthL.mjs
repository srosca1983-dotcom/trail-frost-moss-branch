//#region node_modules/.nitro/vite/services/ssr/assets/permanents-isWOFthL.js
var PERMANENT_SLOT_DEFS = [
	{
		key: "perm-master-a",
		rating: "MASTER",
		title: "Master",
		...
		department: "deck",
		onSheet: true,
		sheetName: "Captain Kluck",
		sortOrder: 10,
		defaultCrewId: "seed-kluck"
	},
	{
		key: "perm-master-b",
		rating: "MASTER",
		...
		department: "deck",
		onSheet: true,
		sheetName: "Captain Tuck",
		sortOrder: 11,
		defaultCrewId: "seed-tuck"
	},
	{
		key: "perm-cm-a",
		rating: "C/M",
	...
	},
	{
		...
		department: "deck",
		onSheet: true,
		sheetName: "C/M Shahbin",
		sortOrder: 21,
		defaultCrewId: "seed-shahbin"
	},
	{
		key: "perm-ce-a",
		rating: "C/E",
		...
		department: "engine",
		onSheet: true,
		sheetName: "C/E Tesson",
		sortOrder: 30,
		defaultCrewId: "seed-tesson"
	},
	{
		key: "perm-ce-b",
		rating: "C/E",
		...
		department: "engine",
		onSheet: true,
		sheetName: "C/E Navarrete",
		sortOrder: 31,
		defaultCrewId: "seed-navarrete"
	},
	{
		key: "perm-1ae-a",
		rating: "1A/E",
		...
		department: "engine",
		onSheet: true,
		sheetName: "1AE Jensen",
		sortOrder: 40,
		defaultCrewId: "seed-jensen"
	},
	{
		key: "perm-1ae-b",
		rating: "1A/E",
	...
	},
	{
		...
		sheetName: "2AE Brown",
		sortOrder: 50,
		defaultCrewId: "seed-brown"
	},
	{
		key: "perm-gas2-a",
		rating: "2 A/E DAY",
		title: "Gas 2 / 2 A/E Day",
		seat: "A",
		unionHall: "MEBA",
		sailingBillet: "24",
		department: "engine",
		onSheet: true,
		sheetName: "Gas 2 Albrecht",
		sortOrder: 60,
		defaultCrewId: "seed-albrecht"
	},
	{
		key: "perm-gas2-b",
		rating: "2 A/E DAY",
		...
		department: "engine",
		onSheet: true,
		sheetName: "Gas 2 Walkup",
		sortOrder: 61,
		defaultCrewId: "seed-walkup"
	},
	{
		key: "perm-bosun-a",
		rating: "BOSUN",
		...
		department: "deck",
		onSheet: true,
		sheetName: "Bosun Gupta",
		sortOrder: 70,
		defaultCrewId: "seed-gupta"
	},
	{
		key: "perm-bosun-b",
		rating: "BOSUN",
		...
		department: "deck",
		onSheet: false,
		sheetName: "Bosun Gonzalez",
		sortOrder: 71,
		defaultCrewId: "seed-gonzalez-joel"
	},
	{
		key: "perm-elec-a",
		rating: "ELECTRICIAN",
		...
		department: "engine",
		onSheet: false,
		sheetName: "Electrician Huffman",
		sortOrder: 80,
		defaultCrewId: "seed-huffman"
	},
	{
		key: "perm-steward-a",
		rating: "STEWARD",
		...
		department: "steward",
		onSheet: true,
		sheetName: "Steward Huyett",
		sortOrder: 90,
		defaultCrewId: "seed-huyett"
	},
	{
		key: "perm-cook-a",
		rating: "COOK",
		...
		department: "steward",
		onSheet: false,
		sheetName: "Cook Scott",
		sortOrder: 91,
		defaultCrewId: "seed-kenya-scott"
	}
];
var PERMANENT_CREW = PERMANENT_SLOT_DEFS.filter((s) => s.defaultCrewId).map((s) => ({
	id: s.defaultCrewId,
...
var RATE_UP_FROM = /* @__PURE__ */ new Set([
	"CM",
	"1AE",
	"2AE",
	"GAS2"
]);
function canRateUp(permanentRating) {
	return RATE_UP_FROM.has(ratingKey(permanentRating));
}
function isRatedUp(permanentRating, sailing) {
...
function upgradeOptions(permanentRating) {
...
function remainingUpgrades(permanentRating, sailing) {
...
function coveringLabel(permanentRating, sailing) {
...
*/
function coveringTripRelief(opts) {
	...
	const holders = forJob.map((s) => s.crewId ? byId.get(s.crewId) : null).filter((p) => Boolean(p));
	if (holders.some((h) => {
	...
	if (opts.people.filter((p) => (p.status ?? "").toLowerCase() === "current" && (p.billetCode ?? "") === code && p.id !== opts.joiningCrewId).some((p) => isRatedUp(p.permanentRating, p.lastPosition))) return null;
	const off = holders.find((h) => (h.status ?? "").toLowerCase() !== "current");
	if (!off) return null;
	return {
		relieving: off.fullName,
		holderId: off.id
	};
}
...
*/
function rotationReturn(opts) {
	const holder = opts.holder;
	if (!holder) return {
		returnOn: null,
		returnFrom: null
	};
	if ((holder.status ?? "").toLowerCase() === "current") return {
		returnOn: null,
		returnFrom: null
	};
	const partner = opts.partners.find((s) => s.sailingBillet && opts.sailingBillet && s.sailingBillet === opts.sailingBillet && s.holder && s.holder.id !== holder.id && (s.holder.status ?? "").toLowerCase() === "current")?.holder;
	if (partner?.dueOff) return {
		returnOn: partner.dueOff,
		returnFrom: partner.fullName
	};
	const sailor = (opts.sailors ?? []).find((p) => (p.status ?? "").toLowerCase() === "current" && opts.sailingBillet && p.billetCode === opts.sailingBillet && p.id !== holder.id && p.lastDueOff);
	if (sailor?.lastDueOff) return {
		returnOn: sailor.lastDueOff,
		returnFrom: sailor.fullName
	};
	return {
		returnOn: null,
		returnFrom: null
...
//#endregion
export { coveringLabel as a, isRatedUp as c, remainingUpgrades as d, rotationReturn as f, canRateUp as i, ratingKey as l, PERMANENT_CREW as n, coveringTripRelief as o, slotDefByKey as p, PERMANENT_SLOT_DEFS as r, eventTypeLabel as s, CHANGE_REASONS as t, reasonLabel as u };
